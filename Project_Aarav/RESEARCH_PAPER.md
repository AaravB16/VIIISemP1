...
1. The small post-dedup dataset limits generalizability claims
2. Only frame-level features are used; CAN signal-level or sequence features may improve results
3. Deep learning models (MLP, LSTM, CNN) were not included in this run
4. Augmentation strategies may not capture all real-world perturbation types (e.g., adversarial attacks)
...
1. **Sequence-based features:** Incorporate sliding-window and recurrence-based features to capture temporal dependencies in CAN traffic.
2. **Deep learning evaluation:** Extend the framework to include MLP, LSTM, and 1D-CNN architectures already scaffolded in the codebase.
3. **Adversarial robustness:** Evaluate model resilience against crafted adversarial CAN frames.
4. **Cross-dataset validation:** Test models trained on CICIoV2024 against other CAN IDS benchmarks (Car-Hacking, OTIDS, SynCAN).
5. **Online learning:** Investigate incremental learning approaches for adapting to evolving attack patterns.
